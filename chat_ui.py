...
def process_turn(message: str, chat_history: List, current_image: Optional[Image.Image]) -> Tuple[List, Optional[Image.Image]]:
    ...
    try:
        ...
        if not data.get("success"):
            chat_history = chat_history + [[message, f"处理失败: {data.get('error', '未知错误')}" ]]
            return chat_history, None

        out_b64 = data.get("final_image")
        if not out_b64:
            chat_history = chat_history + [[message, "未返回结果图像"]]
            return chat_history, None
        ...
        chat_history = chat_history + [[message, f"处理异常: {e}"]]
        return chat_history, None
...
with gr.Blocks(title="图像处理多智能体 - 对话式") as demo:
    gr.Markdown("""
    **图像编辑multi-agent系统**  
    - 右侧上传或更换图片，左侧进行多轮对话指令
    - 示例："请把这张图片转换为灰度图"、"先灰度化再顺时针旋转90度"
    """)
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            ...
            with gr.Accordion("系统状态", open=False):
                status_btn = gr.Button("检查状态")
                status_box = gr.Code(label="Status JSON")
    ...
    status_btn.click(lambda: check_status(), outputs=status_box)
